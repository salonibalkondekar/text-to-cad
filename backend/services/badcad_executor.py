...
class BadCADExecutor:
    ...
    def execute_and_export(
        self, badcad_code: str, model_id: str, validate: bool = True
    ) -> str:
        """
        ...
        if validate:
            ...
            if not is_valid:
                raise BadCADExecutionError(
                    f"Code validation failed: {error_msg}", code=badcad_code
                )

        # Generate STL file path
        stl_path = model_storage.get_temp_file_path(
            prefix=f"model_{model_id}_", suffix=".stl"
        )
        ...
        try:
            ...
            logger.info(
                f"Successfully executed BadCAD code and exported STL: {stl_path}"
            )
            return stl_path
            ...
            try:
                ...
                raise StorageError(
                    f"Failed to create fallback STL: {str(fallback_error)}",
                    "create_fallback",
                )
    ...
    def _execute_badcad_code(self, badcad_code: str) -> Any:
        ...
        try:
            ...
            if model is None:
                ...
                for var_name, var_value in {**exec_locals, **exec_globals}.items():
                    if (
                        ...
                        and hasattr(var_value, "stl")
                        and var_name not in ["badcad"]
                    ):
                ...
                if possible_models:
                    ...
                    logger.warning(
                        f"No 'model' variable found, using '{var_name}' instead"
                    )
                else:
                    available_vars = [
                        k
                        for k in {**exec_locals, **exec_globals}.keys()
                        if not k.startswith("_")
                        and k not in ["badcad", "math", "os", "sys"]
                    ]
                    raise BadCADExecutionError(
            ...
            if isinstance(e, BadCADExecutionError):
                raise
            raise BadCADExecutionError(
                f"Code execution failed: {str(e)}", code=badcad_code
            )
    ...
    def _export_to_stl(self, model_obj: Any, stl_path: str) -> None:
        ...
        try:
            ...
            logger.debug(
                f"STL data type: {type(stl_data)}, size: {len(stl_data) if hasattr(stl_data, '__len__') else 'unknown'}"
            )
    ...
    def validate_execution_environment(self) -> Dict[str, Any]:
        ...
        if self.available:
            try:
                ...
                status["badcad_functions"] = [
                    name for name in dir(badcad) if not name.startswith("_")
                ]
            except Exception as e:
