...
class AICodeGenerator:
    ...
    def _generate_with_gemini(self, prompt: str) -> str:
        ...
        if not self.client:
            raise ConfigurationError(
                "Gemini client not initialized", missing_config="GEMINI_API_KEY"
            )
        ...
        try:
            ...
            if not badcad_code:
                raise AIGenerationError(
                    "No valid code found in AI response",
                    original_error=response_text[:500],
                )
    ...
    def _build_full_prompt(self, user_prompt: str) -> str:
        ...
        user_message += (
            f"Now generate BadCAD code for this prompt: {user_prompt}\\n\\nBadCAD Code:"
        )
...
# The sphere's origin is at its center
outer_bowl = sphere(d=bowl_diameter)
inner_void = sphere(d=bowl_diameter - wall_thickness * 2)

# Subtract the inner from the outer to create a hollow sphere
hollow_sphere = outer_bowl - inner_void

# Create a large cube to cut the hollow sphere in half
# The cube is centered at the origin, then moved up so its bottom is at z=0
cut_box = cube(bowl_diameter, bowl_diameter, bowl_diameter, center=True).move(z=bowl_diameter / 2)

# The final bowl is the bottom half of the hollow sphere
model = hollow_sphere - cut_box""",
            },
            {
                "prompt": "create a pyramid with pentagon base, height is 10 units and each side of pentagon is 5 units",
                "code": """from badcad import *
import math
...
# A regular polygon's circumradius R is related to its side length s by:
# s = 2 * R * sin(pi / n) => R = s / (2 * sin(pi / n))
radius = side_length / (2 * math.sin(math.pi / num_sides))

# Create the pentagonal base using a circle with 5 facets
base = circle(r=radius, fn=num_sides)

# The tip of the pyramid is a very small point
tip = circle(r=0.001)

# Extrude the base to the tip to form the pyramid
model = base.extrude_to(tip, height)""",
            },
            {
                "prompt": "Create a 3D model of a pyramid with a pentagonal base, where each side of the pentagon measures 5 units and the pyramid height is 10 units. Include a cylindrical hole through the center of the pyramid, ensuring the hole fully passes through by making its height greater than that of the pyramid. The hole diameter should be 2 units.",
                "code": """from badcad import *
import math
...
# Extrude the base to the tip to form the pyramid. This creates the pyramid from z=0 to z=height.
pyramid = base.extrude_to(tip, height)

# Create a hole through the center. Make it taller than the pyramid to ensure a clean cut.
# By default, the pyramid is at z=0 to z=height.
# We create a taller cylinder and move it so its center is at the pyramid's center height.
hole = cylinder(d=hole_diameter, h=height + 2, center=True).move(z=height / 2)

# Subtract the hole from the pyramid
model = pyramid - hole""",
            },
        ]
