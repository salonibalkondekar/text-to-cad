...
async def get_user_info(
    request: UserInfoRequest, http_request: Request, response: Response
):
    """
    ...
    try:
        ...
        logger.info(
            f"User data updated for {request.user_id}: {session_data['user']['model_count']} models"
        )
    ...
    try:
        ...
        try:
            result = await analytics_client.increment_user_count(
                user_id=request.user_id, session_cookie=session_cookie
            )

            logger.info(
                f"User {request.user_id} count incremented to: {result['model_count']}"
            )

            return UserCountResponse(success=True, model_count=result["model_count"])

        except httpx.HTTPStatusError as e:
            if e.response.status_code == 403:
                logger.warning(f"User {request.user_id} exceeded limit")
                raise HTTPException(
                    status_code=403, detail="Model generation limit exceeded"
                )
