...
class TestAPIEndpoints:
    ...
    def test_generate_model_success(self):
        ...
        with patch('app.generate_badcad_code_with_gemini') as mock_gemini:
            with patch('app.execute_badcad_and_export') as mock_execute:
                # Mock responses
                mock_gemini.return_value = "from badcad import *\nmodel = cube(10, 10, 10)"
    ...
    def test_download_model_success(self):
        ...
        try:
            ...
            assert f"model_{model_id}.stl" in response.headers.get("content-disposition", "")
        finally:
            # Cleanup
        ...
        with patch('app.load_collected_emails') as mock_load:
            mock_load.return_value = {
                "user1": {
                    ...
                    "model_count": 3,
                    "prompts": [
                        {"prompt": "Make a cube", "type": "generate", "timestamp": "2024-01-01T10:00:00"},
                        {"prompt": "Make a sphere", "type": "generate", "timestamp": "2024-01-01T11:00:00"}
                    ]
                },
                "user2": {
    ...
    def test_add_user_prompt_new_user(self):
        ...
        with patch('app.load_collected_emails') as mock_load:
            with patch('app.save_collected_emails') as mock_save:
                ...
                result = add_user_prompt("new_user", "new@example.com", "Make a cube", "generate")
    ...
    def test_add_user_prompt_existing_user(self):
        ...
        with patch('app.load_collected_emails') as mock_load:
            with patch('app.save_collected_emails') as mock_save:
                ...
                result = add_user_prompt("existing_user", "existing@example.com", "New prompt", "execute_code")
...
```"""
    ...
    def test_gemini_permission_denied(self):
        ...
        with patch('app.GEMINI_AVAILABLE', True):
            with patch('app.gemini_client') as mock_client:
                mock_client.models.generate_content.side_effect = Exception("PERMISSION_DENIED")
    ...
    def test_execute_badcad_success(self):
        ...
        with patch('app.BADCAD_AVAILABLE', True):
            ...
            with patch('builtins.exec') as mock_exec:
                ...
                stl_path = execute_badcad_and_export("from badcad import *\nmodel = cube(10,10,10)", "test_123")
